	...
	while ( $loopn->have_posts() ) {
		...
		if( have_rows('video_post_settings') ):
			while( have_rows('video_post_settings') ): the_row();
				...
				<?php } else { ?>
					<div class="my-product-list card bg-tranparent w-100 m-2 flex-fill">
						<div class="card bg-tranparent w-100">
						<?php if ( $productsimgurl ) { ?>
							<div class="feat-products w-100 bg-media-scroll">
								<img src="<?php echo $productsimgurl; ?>" class="h-100">
							</div>
						<?php } ?>
							<div class="card-body bg-tranparent border-bottom-0">
							<?php if ( $productstitle ) { ?>
								<h6 class="card-title title-2-lines"><?php echo $productstitle; ?></h6>
							<?php } ?>
								<h6 class="price text-right"><span class="woocommerce-Price-amount amount"><bdi><?php echo $product->get_price_html(); ?></bdi></h6>
							</div>
							<div class="card-footer bg-tranparent border-top-0 d-flex">
								<a href="<?php echo $productsurlink; ?>" class="card-link d-inline w-50 text-size-12px p-0">Read more</a>
								<a href="?add-to-cart=<?php echo $productid; ?>" data-quantity="1" class="btn btn-main d-inline w-50 add_to_cart_button ajax_add_to_cart text-size-12px p-0" data-product_id="<?php echo $productid; ?>" data-product_sku="<?php echo $product->get_sku(); ?>" aria-label="Add “<?php echo $productstitle; ?>” to your cart" aria-describedby="" rel="nofollow">Add to cart</a>
							</div>
						</div>
					</div>
	<?php
